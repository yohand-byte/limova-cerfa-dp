...
async function testDozule() {
  try {
    console.log('=== TEST CERFA DOZULE ===');
    console.log('');
    
    const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY;
    
    if (!GOOGLE_API_KEY) {
      console.error('ERREUR: GOOGLE_API_KEY manquante');
      console.log('Utilisez: export GOOGLE_API_KEY="votre_cle"');
      process.exit(1);
    }
    
    const address = '14 rue emile nicol, 14430 dozule';
    ...
    const fields = {
      ...
      'D2R_raison': installer.legalName,
      'D2S_siret': installer.siret,
      'D2J_type': installer.companyType,
    ...
    const outputName = `CERFA_DOZULE_${dateStr}.pdf`;
    ...
    console.log('  - Case agrivoltaique Non: cochee');
